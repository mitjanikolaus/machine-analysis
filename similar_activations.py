...
def run_analysis(rnn_type, similar_input_criterium, other_input_criterium, models_to_evaluate, model_part_to_evaluate, compare, print_outliers=True, legend_labels=('similar input','other input'), legend_location=(2,2)):
    ...
    :param models_to_evaluate: which models of the model zoo to evaluate (1 up to 5)
    :param model_part_to_evaluate: which model part to perform the similarity analysis on
    :param compare: COMPARE_SAMPLES or COMPARE_TIMESTEPS
    :param print_outliers: if True, outliers in similarity for each timestep are printed to the console
    :param legend_labels: labels for the legend of the plot
    :param legend_location: location of the legends of the plot
    """
...
def run_models_and_evaluate(activation_data_similar, activation_data_other, compare, print_outliers=True):
    ...
    for ts in range(len(distances_similar_each_timestep)):
        if print_outliers:
            print_outliers_for_timestep(distances_different_each_timestep, distances_similar_each_timestep, ts)
...
def print_outliers_for_timestep(distances_different_each_timestep, distances_similar_each_timestep, ts):
    ...
    outliers_similar_low_change = np.where(mean_distances_similar_per_cell < (
                np.mean(mean_distances_similar_per_cell) - 2 * np.std(mean_distances_similar_per_cell)))[0]
    outliers_different_low_change = np.where(mean_distances_different_per_cell < (
                np.mean(mean_distances_different_per_cell) - 2 * np.std(mean_distances_different_per_cell)))[0]
    outliers_similar_high_change = np.where(mean_distances_similar_per_cell > (
                np.mean(mean_distances_similar_per_cell) + 2 * np.std(mean_distances_similar_per_cell)))[0]
    outliers_different_high_change = np.where(mean_distances_different_per_cell > (
                np.mean(mean_distances_different_per_cell) + 2 * np.std(mean_distances_different_per_cell)))[0]
